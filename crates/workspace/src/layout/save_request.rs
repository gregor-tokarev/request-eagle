...
pub(crate) fn open(
    ...
    let dialog = cx.new(|cx| {
        ...
        let selected = (destinations.len() == 1).then_some(0);
        SaveRequestDialog {
            main,
            sidebar,
            tab_id,
...
impl Render for SaveRequestDialog {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child("Save to")
            .child(Input::new(&self.filter).aria_label("Search collections and folders"))
            .child(
                v_flex()
                    ...
                    .rounded_md()
                    .when(self.destinations.is_empty(), |list| {
                        list.child(
                            div()
                                .p_3()
                                .text_color(cx.theme().muted_foreground)
                                .child("Create a collection to save this request."),
                        )
                    })
                    .children(
                        self.destinations
                            ...
                            .filter(|(_, destination)| {
                                destination.label.to_lowercase().contains(&query)
                            })
                            .map(|(index, destination)| {
                                Button::new(("save-destination", index))
                                    .debug_selector(move || format!("save-destination-{index}"))
                                    .ghost()
                                    .w_full()
                                    .justify_start()
